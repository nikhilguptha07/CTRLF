...
export const AdminCamerasTab: React.FC = () => {
  ...
  const handleDeleteCamera = async (camera: CameraRecord) => {
    if (!window.confirm(`Are you sure you want to delete camera ${camera.name}? This action cannot be undone.`)) return;
    try {
      await apiClient.deleteAdminCamera(camera.id);
      await fetchCameras();
    } catch (err: any) {
      alert(`Failed to delete camera: ${err?.message || err}`);
    }
  };
