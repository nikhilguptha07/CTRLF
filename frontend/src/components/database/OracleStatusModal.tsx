...
export const OracleStatusModal: React.FC<OracleStatusModalProps> = ({
  ...
  return (
    <div id="oracle-modal" data-database="true" className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-md p-4 animate-fade-in font-sans">
      <div 
        className="relative w-full max-w-lg bg-white/98 backdrop-blur-2xl border border-slate-200/90 rounded-2xl shadow-2xl overflow-hidden text-slate-800 animate-scale-in"
      >
          ...
          <div className="flex items-center gap-3.5 mb-2">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="px-2 py-0.5 text-[10px] font-mono font-bold bg-emerald-50 text-emerald-700 border border-emerald-200 rounded flex items-center gap-1.5">
        ...
        <div className="px-6 pb-4">
          <div className="p-4 rounded-xl bg-slate-50/90 border border-slate-200/80 space-y-3">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <span className="text-xs font-mono font-bold text-emerald-700 bg-emerald-50 border border-emerald-200/80 px-2 py-0.5 rounded flex items-center gap-1">
                ...
                <button
                  ...
                  disabled={isChecking}
                  className="p-1.5 text-slate-500 hover:text-slate-800 hover:bg-slate-100 rounded-lg transition-all cursor-pointer disabled:opacity-50 border border-slate-200 shadow-2xs"
        ...
        <div className="px-6 pb-4 space-y-2">
          <div className="flex items-center justify-between text-xs">
            ...
            <span className="text-[10px] font-mono px-2 py-0.5 bg-slate-100 border border-slate-200 text-slate-500 rounded">
          ...
          <div className="divide-y divide-slate-100 rounded-xl bg-white border border-slate-200/80 overflow-hidden shadow-2xs">
            {schemaTables.map((tbl) => (
              <div key={tbl.name} className="p-2.5 px-3 flex items-center justify-between text-xs hover:bg-slate-50/70 transition-colors">
                ...
                <span className="text-[10px] font-mono font-bold px-2 py-0.5 rounded bg-slate-100 text-slate-700 border border-slate-200/80 shrink-0">
                  {tbl.records}
                </span>
              </div>
            ))}
          ...
          <div className="p-3 rounded-xl bg-indigo-50/70 border border-indigo-100 flex items-center justify-between text-xs">
            ...
            {onOpenAuditLogs && (
              <button
                ...
                className="px-2.5 py-1 bg-white hover:bg-indigo-50 text-indigo-700 border border-indigo-200 rounded-lg text-[11px] font-semibold transition-all shadow-2xs flex items-center gap-1 cursor-pointer shrink-0"
              >
        ...
        <div className="px-6 py-3.5 bg-slate-50 border-t border-slate-100 flex items-center justify-between text-xs text-slate-500">
          ...
          <button
            type="button"
            onClick={onClose}
            className="px-4 py-1.5 bg-slate-900 hover:bg-slate-800 text-white rounded-lg text-xs font-semibold transition-all cursor-pointer shadow-xs"
          >
            Done
          </button>
        </div>
